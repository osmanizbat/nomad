...
- `max` - The maxim running count of the targeted resource. This can be 0 or any
  positive integer.
...
- `cooldown` - A time interval after a scaling action during which no additional
  ...
  (i.e.: "5s", "1m" etc). If omitted the configuration value
...
- `evaluation_interval` - Defines how often the policy is evaluated by the
  Autoscaler. It should be provided as a duration (i.e.: "5s", "1m" etc). If
