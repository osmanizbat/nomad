...
supporting the operations of the Nomad Autoscaler. The Technical Preview has
the additional requirement for the read-job ACL capability. Therefore, the
following policy is sufficient for creating an ACL token that can be used by
the autoscaler when not using the Nomad APM:
...
When using the Nomad APM for metrics, the Nomad Autoscaler requires additional
permissions in order to call the required API endpoints. The Nomad APM for job
task group metrics requires the following policy:
...
When using the Nomad APM to provide cluster scaling metrics, the following policy
is needed:
...
Any token created using this policy should be provided in the nomad configuration,
as described below.
